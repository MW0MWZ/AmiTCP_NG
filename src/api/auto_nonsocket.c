...
/*
 * auto_nonsocket.c --- AUTODOC source (API reference documentation, not code).
 *
 * This file contains no functions. It holds the "nonsocket"-group bsdsocket.library
 * reference pages in Commodore AutoDoc format (the six-asterisk "name" blocks
 * below), from which a documentation tool extracts the printed/AmigaGuide manual.
 * It is listed in the build so the docs travel with the source; it compiles to an
 * essentially empty object. To learn an API function's exact semantics, read its
 * page here. (PORT (AmiTCP_NG): embedded comment markers in the examples were neutralised so
 * gcc's stricter comment handling accepts the file -- see PORTING.md.)
 */
